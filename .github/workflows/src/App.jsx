...
function normalizeDisc(t){
  return {
    photos: [], cover: '', favorite:false, watched:false, dateAcquired:'', purchaseLocation:'',
    purchasePrice:'', discCondition:'', sleeveCondition:'', inserts:'', rating:'', tags:'', timelineNote:'',
    ...t,
...
function mainImage(t, photoLibrary = {}){
  const disc = normalizeDisc(t);
  if(disc.cover && String(disc.cover).startsWith('data:')) return disc.cover;
  if(disc.cover && photoLibrary[disc.cover]) return photoLibrary[disc.cover];
  const first = disc.photos?.[0];
  if(!first) return '';
  return resolvePhotoSrc(first, photoLibrary);
}

function archiveId(t){
  return String(t?.id || '').replace('LaserDisc-', 'LaserDisc-');
}
...
function badgeList(t){
  ...
  if(/2-disc|2 disc|double/i.test(text)) badges.push('2 TAPE');
...
function DiscCard({ disc, onOpen, mini=false, photoLibrary={} }){
  const img = mainImage(disc, photoLibrary);
  const special = /screener|collector|special|custom|nintendo|disney parks/i.test(disc.edition || '');
  const badges = badgeList(disc);
  return (
    <article className={`disc-card ${mini ? 'mini-card':''}`} onClick={(e) => onOpen(disc.id, e.currentTarget)}>
      <div className={`cover ${img ? 'has-img':''}`}>
        {img ? <img src={img} alt={`${disc.title} cover`} /> : <div className="cover-title"><span className="coming-soon">PHOTO NEEDED</span><br />{disc.title}</div>}
        <div className="case-shine"></div>
      </div>
      <div className="meta">
        <div className="title">{disc.title}</div>
        <div className="small">{disc.vhsYear || 'No year'} • {disc.studio || 'Unknown'}</div>
        <div className="badge-row">
          <span className={`tag ${special ? 'alt':''}`}>{disc.packaging}</span>
...
function Shelf({ title, subtitle, discs, onOpen, photoLibrary={} }){
    ...
    <section className="shelf-section">
      ...
      <div className={shelfClassName}>
        <div ref={shelfRef} className="shelf" aria-label={`${title} shelf`}>
          {discs.length
            ? discs.map(t => <DiscCard key={t.id} disc={t} onOpen={onOpen} mini photoLibrary={photoLibrary} />)
...
export default function App(){
  const [discs,setDiscs] = useState(() => {
    const saved = localStorage.getItem('noahVhs6_discs');
    return saved ? JSON.parse(saved).map(normalizeDisc) : STARTER_TAPES.map(normalizeDisc);
  });
  ...
  const [view,setView] = useState(() => sessionStorage.getItem('laserdiscArchive_lastView') || 'home');
  ...
  const [selectedId,setSelectedId] = useState(() => sessionStorage.getItem('laserdiscArchive_selectedDisc') || null);
  const [selectedPhoto,setSelectedPhoto] = useState(0);
  const [openingDisc,setOpeningDisc] = useState(null);
  ...
  const [musicEnabled,setMusicEnabled] = useState(() => localStorage.getItem('laserdisc_music_enabled') === 'true');
  const [sfxEnabled,setSfxEnabled] = useState(() => localStorage.getItem('laserdisc_sfx_enabled') !== 'false');
  ...
  useEffect(()=>{localStorage.setItem('noahVhs6_discs', JSON.stringify(discs));},[discs]);
  ...
  useEffect(()=>{sessionStorage.setItem('laserdiscArchive_lastView', view);},[view]);
  useEffect(()=>{
    if(selectedId) sessionStorage.setItem('laserdiscArchive_selectedDisc', selectedId);
  },[selectedId]);
  ...
  useEffect(() => {
    ...
    window.addEventListener('appinstalled', () => {
      ...
      notify('Noah\'s LaserDisc Archive installed.');
    });
  ...
  function toggleMusic(){
    ...
    localStorage.setItem('laserdisc_music_enabled', String(next));
  ...
  function toggleSfx(){
    ...
    setSfxEnabled(next);
    localStorage.setItem('laserdisc_sfx_enabled', String(next));
  ...
  function saveScrollPosition(page = view){
    ...
    sessionStorage.setItem(`laserdiscArchive_scroll_${page}`, String(y));
  }

  function restoreScrollPosition(page){
    const saved = scrollPositions[page] ?? Number(sessionStorage.getItem(`laserdiscArchive_scroll_${page}`) || 0);
    setTimeout(() => {
      const el = scrollAreaRef.current;
  ...
  function goToView(nextView, options = {}){
    if(nextView === 'browse'){
      if(options.resetBrowse || view === 'browse'){
        ...
        sessionStorage.setItem('laserdiscArchive_scroll_browse', '0');
      }
    }

    if(view === nextView){
      scrollAreaRef.current?.scrollTo({top:0, behavior:'smooth'});
      sessionStorage.setItem(`laserdiscArchive_scroll_${nextView}`, '0');
    ...
    setView(nextView);
    sessionStorage.setItem('laserdiscArchive_lastView', nextView);
    setTimeout(() => {
  ...
  function goBackInApp(){
    ...
    if(previous){
      saveScrollPosition(view);
      setView(previous);
      sessionStorage.setItem('laserdiscArchive_lastView', previous);
  ...
  function openDisc(id, sourceEl = null){
    const disc = discs.find(t => t.id === id);
    const img = disc ? mainImage(disc, photoLibrary) : '';

    const switchToDetail = () => {
      saveScrollPosition(view);
      viewHistoryRef.current.push(view);
      try{ window.history.pushState({vhsView:'detail'}, '', window.location.href); }catch(error){}
      sessionStorage.setItem('laserdiscArchive_selectedDisc', id);
      sessionStorage.setItem('laserdiscArchive_lastView', 'detail');
      setSelectedId(id);
      setSelectedPhoto(0);
      setEditOpen(false);
      setView('detail');
    ...
    if(sourceEl && disc && !window.matchMedia('(prefers-reduced-motion: reduce)').matches){
  ...
  const selected = discs.find(t => t.id === selectedId);
...
function pickMovieNight(){
    ...
    const choice = discs[Math.floor(Math.random() * discs.length)];
    const pool = discs.filter(t => t.id !== choice.id).sort(() => 0.5 - Math.random());
  ...
  const genreOptions = useMemo(() => {
    ...
    const fromDiscs = discs.map(t => t.genre).filter(Boolean);
    return [...new Set([...base, ...fromDiscs])].sort((a,b)=>a.localeCompare(b));
  }, [discs]);

  const discsMissingPhotos = useMemo(() => discs.filter(t => !mainImage(t, photoLibrary)), [discs, photoLibrary]);

  const filtered = useMemo(() => {
    let list = discs.filter(t => [t.title,t.studio,t.edition,t.packaging,t.notes,t.genre,t.tags,t.vhsYear].join(' ').toLowerCase().includes(query.toLowerCase()));
  ...
  }, [discs, query, pkg, edition, missingPhotosOnly, photoLibrary]);

  const stats = {
    total: discs.length,
    photos: discs.filter(t => mainImage(t, photoLibrary)).length,
    photosNeeded: discs.filter(t => !mainImage(t, photoLibrary)).length,
    favorites: discs.filter(t => t.favorite).length,
    watched: discs.filter(t => t.watched).length,
    screeners: discs.filter(t => has(t,/screener/i)).length,
    specials: discs.filter(t => has(t,/collector|special|lenticular|metallic|2-disc|blue disc/i)).length,
    clamshells: discs.filter(t => /clamshell/i.test(t.packaging || '')).length,
    sleeves: discs.filter(t => /sleeve/i.test(t.packaging || '')).length,
  };

  function updateDisc(id, patch){
    setDiscs(prev => prev.map(t => t.id === id ? normalizeDisc({...t, ...patch}) : t));
  }
  ...
  async function saveDiscPhoto(discId, label, file){
    if(!file) return null;
    const src = await compressImage(file);
    const photoId = `${discId}-${Date.now()}-${Math.random().toString(36).slice(2,8)}`;
    try{
  ...
  async function addPhoto(label, file){
    ...
    sessionStorage.setItem('laserdiscArchive_selectedDisc', selected.id);
    sessionStorage.setItem('laserdiscArchive_lastView', 'detail');
    try{
      const photo = await saveDiscPhoto(selected.id, label, file);
      if(!photo) return;
      const photos = [...(selected.photos || []), photo];
      updateDisc(selected.id, {photos, cover: selected.cover || (photo.photoId || photo.src)});
  ...
  function removePhoto(i){
    ...
    updateDisc(selected.id, {photos, cover});
  ...
  function saveEdit(){
    ...
    updateDisc(selected.id, {...cleanForm, genre: nextGenre});
    setForm({...cleanForm, genre: nextGenre, customGenre:''});
    setEditOpen(false);
    notify('Disc details saved.');
  }

  function quickAddDiscs(){
    const rows = quickRows.split('\n').map(r => r.trim()).filter(Boolean);
    if(!rows.length){ notify('Paste one disc per line first.'); return; }
    const additions = rows.map((row, idx) => {
      const parts = row.split(',').map(p => p.trim());
      const title = parts[0] || 'Untitled Disc';
      ...
      return normalizeDisc({
        id: 'LaserDisc-' + String(discs.length + idx + 1).padStart(4,'0'),
        title,
    ...
    setDiscs(prev => [...prev, ...additions]);
    setQuickRows('');
    notify(`${additions.length} disc${additions.length>1?'s':''} added.`);
    goToView('browse');
  }
  ...
  async function addDisc(e){
    ...
    const newDisc = normalizeDisc({
      ...
      discCondition: fd.get('discCondition') || '',
      notes: fd.get('notes') || '',
      photos: savedPhotos,
    ...
    form.reset();
    notify(savedPhotos.length ? 'Disc added with photos.' : 'Disc added.');
    setSelectedId(next);
    goToView('detail');
  }
  ...
  async function exportBackup(){
    ...
    for(const disc of discs){
      for(const p of (disc.photos || [])){
        if(p.photoId){
          photos[p.photoId] = photoLibrary[p.photoId] || await idbGetPhoto(p.photoId).catch(() => '');
        }
      }
    }
    const blob = new Blob([JSON.stringify({version:'7.5', discs, wishlist, photos}, null, 2)], {type:'application/json'});
  ...
  async function importBackupFile(file){
    ...
    try{
      ...
      const importedDiscs = Array.isArray(data) ? data : data.discs;
      ...
      const confirmed = window.confirm(
        `Import backup with ${importedDiscs.length} discs? This will replace the current archive on this device.`
      );
      if(!confirmed) return;
      ...
      for(const [id, src] of Object.entries(importedPhotos)){
        ...
        try{
          ...
          // If IndexedDB fails, the disc records can still restore;
          // old backups with embedded src photos will still display.
        }
      }

      setPhotoLibrary(prev => ({...prev, ...newPhotoLibrary}));
      setDiscs(importedDiscs.map(normalizeDisc));
      setWishlist(importedWishlist);
      setSelectedId(null);
      setSelectedPhoto(0);
      setViewerPhoto(null);
      setView('home');
      sessionStorage.removeItem('laserdiscArchive_selectedDisc');
    ...
    <>
      <header className="app-header" onClick={() => goToView('home')} role="button" title="Back to top">
        <div className="header-inner">
          <img className="header-ticket-logo" src="./vhs-ticket-header-logo-user.png" alt="LaserDisc Archive logo" />
          <div><h1>LASERDISC ARCHIVE</h1><div className="sub">Catalog. Collect. Preserve.</div><div className="version-badge">v1.0</div></div>
        </div>
      </header>
      ...
      <main ref={scrollAreaRef} className="app-scroll">
        {view === 'home' && (
          <>
            <section className="hero">
              ...
              <div className="stats">
                <div className="stat"><strong>{stats.total}</strong><span>Total Discs</span></div>
                ...
                <div className="stat"><strong>{stats.specials}</strong><span>Special Discs</span></div>
            ...
            <Shelf title="New Arrivals" subtitle="Latest archive IDs" discs={[...discs].slice(-12).reverse()} onOpen={openDisc} photoLibrary={photoLibrary}/>
            <Shelf title="Staff Picks" subtitle="Favorites" discs={discs.filter(t=>t.favorite).slice(0,16)} onOpen={openDisc} photoLibrary={photoLibrary}/>
            <Shelf title="Special Shelf" subtitle="Screeners & variants" discs={discs.filter(t=>has(t,/screener|collector|special|nintendo|disney parks|custom|lenticular|metallic|2-disc|blue disc/i)).slice(0,16)} onOpen={openDisc} photoLibrary={photoLibrary}/>
            <Shelf title="Nintendo / Promo Shelf" subtitle="Game discs & promos" discs={discs.filter(t=>has(t,/nintendo|donkey kong|pokemon|pokémon/i)).slice(0,16)} onOpen={openDisc} photoLibrary={photoLibrary}/>
            <Shelf title="Disney Shelf" subtitle="Disney, Pixar & Parks" discs={discs.filter(t=>has(t,/disney|pixar|walt disney|parks|goofy|toy story/i)).slice(0,16)} onOpen={openDisc} photoLibrary={photoLibrary}/>
            <Shelf title="Photo Shelf" subtitle="Your real disc photos" discs={discs.filter(t=>mainImage(t, photoLibrary)).slice(0,14)} onOpen={openDisc} photoLibrary={photoLibrary}/>
          </>
        )}
        ...
        {view === 'browse' && (
          <>
            ...
            <div className="small" style={{margin:'8px 2px 14px'}}>{filtered.length} discs showing • Title A-Z {missingPhotosOnly ? '• Missing photos only' : ''}</div>
            {missingPhotosOnly && !filtered.length ? <div className="panel empty-state"><h3>You're up to date</h3><p className="small">Every disc currently has at least one photo.</p></div> : <div className="grid">{filtered.map(t => <DiscCard key={t.id} disc={t} onOpen={openDisc} photoLibrary={photoLibrary}/>)}</div>}
          </>
        )}
          ...
          <section>
            ...
            <div className="detail-layout" style={{marginTop:14}}>
              ...
              <div className="detail-card">
                ...
                <div className="actions">
                  <button onClick={()=>updateDisc(selected.id,{favorite:!selected.favorite})}>⭐ {selected.favorite ? 'Unfavorite':'Favorite'}</button>
                  <button className="secondary" onClick={()=>updateDisc(selected.id,{watched:!selected.watched})}>▶ {selected.watched ? 'Unwatch':'Watched'}</button>
                </div>
                {[
                  ['Archive ID',archiveId(selected)],['LaserDisc Release',selected.vhsYear || 'No year listed'],['Studio',selected.studio],['Edition',selected.edition],['Packaging',selected.packaging],['Genre',selected.genre],['Disc Condition',selected.discCondition || selected.condition || 'Not set'],['Sleeve Condition',selected.sleeveCondition || 'Not set'],['Inserts',selected.inserts || 'Not set'],['Rating',selected.rating ? selected.rating + '/5':'Not rated'],['Acquired',selected.dateAcquired || 'Not set'],['Found At',selected.purchaseLocation || 'Not set'],['Price',selected.purchasePrice ? '$'+selected.purchasePrice:'Not set'],['Collector Badges',badgeList(selected).join(', ') || 'None'],['Tags',selected.tags || ''],['Notes',selected.notes || '']
                ].map(([a,b])=><div className="row" key={a}><span>{a}</span><span>{b}</span></div>)}

                <div className="panel">
                  <h3>LaserDisc Photos</h3>
                  <p className="small">Use your camera for front cover, back cover, spine, disc label, or choose from gallery.</p>
                  <div className="photo-capture-tip">
                    <strong>Photo tip:</strong> place the disc on a plain, high-contrast background and fill the frame. New photos are stored in IndexedDB for larger collections, with compression kept on for stability.
                  </div>
                  <div className="photo-buttons">
                    {['Front Cover','Back Cover','Spine','Disc Label'].map(label => (
                      <label key={label}>
                ...
                <button className="secondary" onClick={beginEdit}>Edit LaserDisc Details</button>
                <div className={`panel edit-panel ${editOpen ? 'show':''}`}>
                  <h3>Edit Details</h3>
                  <div className="formgrid">
                    {['title','vhsYear','studio','edition','packaging','genre','discCondition','sleeveCondition','inserts','rating','dateAcquired','purchaseLocation','purchasePrice','tags'].map(f=>(
                      <React.Fragment key={f}>
                        ...
                        {f === 'genre' ? (
                          ...
                          <select value={form.edition || 'Standard'} onChange={e=>setForm({...form, edition:e.target.value})}>
                            <option>Standard</option><option>Widescreen</option><option>Screener</option><option>Collector's Edition</option><option>Special Edition</option><option>Custom Disc</option><option>Nintendo Power</option><option>Disney Parks</option>
                          </select>
                        ) : f === 'discCondition' || f === 'sleeveCondition' ? (
        ...
        {view === 'add' && (
          <form className="panel formgrid" onSubmit={addDisc}>
            <h3>Add a Disc</h3>
            <div className="quick-add-box">
              <h3>Quick Add Stack</h3>
              <p className="small">Paste one disc per line when you come home with a stack. You can clean up photos/details later.</p>
              ...
              <button type="button" className="secondary" onClick={quickAddDiscs}>Quick Add These Discs</button>
            </div>
            <label>Title</label><input name="title"/>
            <label>LaserDisc Release Year</label><input name="vhsYear"/>
            <label>Studio / Distributor</label><input name="studio"/>
            <label>Edition</label><select name="edition"><option>Standard</option><option>Widescreen</option><option>Screener</option><option>Collector's Edition</option><option>Special Edition</option><option>Custom Disc</option><option>Nintendo Power</option><option>Disney Parks</option></select>
            ...
            <label>New Genre (optional)</label><input name="customGenre" placeholder="Add a new genre for this disc"/>
            <label>Disc Condition</label><select name="discCondition"><option></option><option>Mint</option><option>Excellent</option><option>Very Good</option><option>Good</option><option>Fair</option><option>Poor</option></select>
            <label>Notes</label><textarea name="notes" rows="4"/>
            <div className="add-photo-box"><h3>Photos While Adding</h3><p className="small">Optional, but handy if you want the disc fully logged right away.</p><label>Front Cover</label><input name="photoFront" type="file" accept="image/*" capture="environment"/><label>Back Cover</label><input name="photoBack" type="file" accept="image/*" capture="environment"/><label>Spine</label><input name="photoSpine" type="file" accept="image/*" capture="environment"/><label>Disc Label</label><input name="photoDisc" type="file" accept="image/*" capture="environment"/></div>
            <button>Add LaserDisc</button>
          </form>
        )}
        ...
        {view === 'timeline' && (
          <section className="panel">
            ...
            <p className="small">Add acquisition dates to discs and your finds appear here by date.</p>
            <div className="timeline">
              {discs.filter(t=>t.dateAcquired).sort((a,b)=>String(b.dateAcquired).localeCompare(String(a.dateAcquired))).map(t=>(
                <div className="tl-item" key={t.id} onClick={()=>openDisc(t.id)}>
        ...
        {view === 'stats' && (
          <>
            ...
            <div className="panel">
              <h3>Backup</h3>
              <p className="small">Back up your collection including photos, edits, added discs, and wishlist.</p>
              ...
              <div className="restore-box">
                ...
                />
                <p className="small">Choose a backup JSON file to restore discs, wishlist, and included photos.</p>
              </div>
            </div>
            <div className="panel music-settings-panel">
              <h3>Audio Settings</h3>
              <p className="small">Turn on the looping LaserDisc Archive background theme. Music pauses when the app is backgrounded and tries to resume when the app reopens.</p>
      ...
      {openingDisc && (
        <div className="disc-open-stage transparent-cover-stage" aria-hidden="true">
          <div
            className="disc-open-overlay transparent-cover-open"
            style={{
              '--start-x': `${openingDisc.x}px`,
              '--start-y': `${openingDisc.y}px`,
              '--start-w': `${openingDisc.w}px`,
              '--start-h': `${openingDisc.h}px`,
              '--move-x': `${openingDisc.moveX}px`,
              '--move-y': `${openingDisc.moveY}px`,
              '--open-scale': openingDisc.scale,
              left: 0,
              top: 0,
              width: openingDisc.w,
              height: openingDisc.h
            }}
          >
            {openingDisc.img ? <img src={openingDisc.img} alt="" /> : <div className="disc-open-title">{openingDisc.title}</div>}
          </div>
        </div>
      )}
      ...
      {movieNight && (
        <div className={`movie-night-overlay feature-night-overlay ${movieNight.stage}`}>
          <div className="feature-night-card">
            <div className="movie-night-kicker">LASERDISC ARCHIVE PRESENTS</div>
            ...
            <div className="feature-reel-window">
              ...
              >
                {movieNight.reel.map((t, index) => (
                  <div className={`feature-reel-disc ${movieNight.stage !== 'spin' && index === movieNight.winnerIndex ? 'winner':''}`} key={`${t.id}-${index}`}>
        ...
        <div className="photo-viewer-overlay">
          <div className="photo-viewer-card zoomable">
            <div className="photo-viewer-head">
              <strong>{viewerPhoto.label || 'LaserDisc Photo'}</strong>
            ...
            >
              <img
                src={viewerPhoto.src}
                alt={viewerPhoto.label || 'Disc photo'}
